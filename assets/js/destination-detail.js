document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('detailTips').innerText = dest.tips || 'Chuẩn bị hành lý phù hợp với thời tiết địa phương.';
  ...
  const activities = dest.activities && dest.activities.length > 0 ? dest.activities : [
    { time: "Sáng (07:30 - 11:30)", title: `Tham quan ${dest.name}`, cost: Math.round(dest.avgCostPerDay * 0.35) },
    { time: "Trưa (12:00 - 13:30)", title: `Thưởng thức ẩm thực local tại ${dest.city}`, cost: Math.round(dest.avgCostPerDay * 0.25) },
    { time: "Chiều (14:30 - 17:30)", title: `Khám phá & chụp ảnh check-in`, cost: Math.round(dest.avgCostPerDay * 0.20) },
    { time: "Tối (18:30 - 22:00)", title: `Dạo phố đêm ngắm cảnh`, cost: Math.round(dest.avgCostPerDay * 0.20) }
  ];
  ...
  if (favBtn) {
    ...
    favBtn.addEventListener('click', async () => {
      ...
      showToast(updated ? 'Đã lưu yêu thích!' : 'Đã xóa khỏi yêu thích!', 'info');
    });
  }
...
async function renderReviews(destId) {
  ...
  container.innerHTML = reviews.map(r => `
    <div style="padding: 1.25rem 0; border-bottom: 1px solid var(--border-light);">
      ...
      <div style="font-size: 0.75rem; color: var(--text-subtle); margin-top: 0.35rem;">Ngày: ${escapeHtml(r.date)}</div>
    </div>
  `).join('');
}
...
function initReviewForm(destId) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    form.reset();
    showToast('Đã đăng đánh giá!', 'success');
  });
}