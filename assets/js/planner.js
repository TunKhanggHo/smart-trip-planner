...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function resolveDestinationObject(destId) {
    if (destId === 'custom') {
      const customName = (customInput && customInput.value.trim()) ? customInput.value.trim() : 'Điểm Đến Tự Chọn';
      return {
        id: 'custom',
        name: customName,
        city: customName,
    ...
    const selectedOption = destSelect ? destSelect.options[destSelect.selectedIndex] : null;
  ...
  if (btnAI) {
    btnAI.addEventListener('click', async () => {
      ...
      const budget = parseInt(document.getElementById('planBudget').value) || 2500000;
      ...
      if (loadingTitle) loadingTitle.innerText = '🤖 Trí tuệ nhân tạo Gemini AI đang sáng tạo lịch trình...';
      if (loadingDesc) loadingDesc.innerText = 'Có thể mất tới 30-40 giây, vui lòng không tắt trang trong lúc chờ...';
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (resData.status === 'success' && resData.data && resData.data.schedule) {
            const aiSchedule = resData.data.schedule.map((day, dIdx) => ({
              dayNumber: day.dayNumber || (dIdx + 1),
              dayTotal: day.dayTotal || (day.activities ? day.activities.reduce((s, a) => s + (parseInt(a.cost) || 0), 0) : 750000),
            ...
            const aiTotalCost = aiSchedule.reduce((sum, d) => sum + d.dayTotal, 0);
            ...
            showToast('✨ Lịch trình được sáng tạo thành công bởi Google Gemini AI!', 'success');
            return;
          }
        }
      } catch (err) {
      ...
      resultArea.style.display = 'none';
      showToast('❌ Không thể tạo lịch trình bằng AI lúc này. Vui lòng kiểm tra kết nối mạng và thử lại!', 'error');
    });
  }
  ...
  function renderGeneratedResult() {
    ...
    if (costDisplay) costDisplay.innerText = `${formatVND(totalPerPerson)} / người`;
    
    const budgetStatus = document.getElementById('resBudgetStatus');
    const isAI = currentGeneratedTrip.generatedBy === 'gemini_ai';
    const badgeAI = isAI ? `<span class="badge" style="background: linear-gradient(135deg, #6366f1, #ec4899); color: white; margin-left: 0.5rem; font-size: 0.8rem;">🤖 Sáng tạo bởi Google Gemini AI</span>` : '';
    ...
    container.innerHTML = currentGeneratedTrip.schedule.map(day => `
  ...
  window.removeActivity = (dayNum, actId) => {
    const day = currentGeneratedTrip.schedule.find(d => d.dayNumber === dayNum);
    if (day) {
      day.activities = day.activities.filter(a => a.id !== actId);
      day.dayTotal = day.activities.reduce((sum, a) => sum + a.cost, 0);
      currentGeneratedTrip.totalCostPerPerson = currentGeneratedTrip.schedule.reduce((sum, d) => sum + d.dayTotal, 0);
      renderGeneratedResult();
  ...
  document.getElementById('btnSaveTrip').addEventListener('click', async () => {
    ...
    if (savedToServer) {
      ...
      showToast('Đã lưu chuyến đi (chế độ tạm thời, backend không phản hồi)!', 'info');
    }
