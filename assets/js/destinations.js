 ...
 * NGƯỜI PHỤ TRÁCH: Khiêm (STT 2 - Khám phá địa điểm & Bộ lọc thông minh)
 * ==============================================================================
 * Kết nối PHP API & MySQL:
 * - Lọc địa điểm theo thể loại, tìm kiếm tức thì, lọc ngân sách, sắp xếp.
 * - Thả tim yêu thích (Favorite) lưu trực tiếp vào CSDL MySQL.
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  catTabsContainer.innerHTML = categories.map(c => `
  ...
  async function render() {
    ...
    if (resultCountBadge) resultCountBadge.innerText = 'Đang tải dữ liệu từ CSDL...';

    // Đã tự động kéo dữ liệu tươi nhất từ MySQL thông qua TripDataService đã nâng cấp
    const results = await TripDataService.filterDestinations(currentCategory, search, budget, sort);
  ...
  window.toggleDestinationFav = async (e, destId) => {
    ...
    showToast(isNowFav ? 'Đã lưu vào danh sách yêu thích!' : 'Đã xóa khỏi danh sách yêu thích!', 'info');
  };
